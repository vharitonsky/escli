...
Future<String> compgen(List<String> arguments) async {
  ...
  if (arguments[0] == 'select') {
    ...
    if (arguments.length == 2) {
      return names
          .where((element) => element.startsWith(arguments[1]))
          .toList()
          .join(' ');
    }
    return names.join(' ');
  ...
  if (arguments[0] == 'index') {
    ...
    if (arguments.length == 2) {
      ...
      if (baseHost != '') {
        var allIndices =
            (await indices.getIndices(baseHost)).map((e) => e.name).toList();
        return allIndices
            .where((element) => element.startsWith(arguments[2]))
            .toList()
            .join(' ');
      }
      // escli index
  ...
  if (arguments.length == 1) {
    return COMMANDS
        .where((element) => element.startsWith(arguments[0]))
        .join(' ');
  }
